...
class WebsiteCardProducts(http.Controller):
    ...
    @http.route(['/nuestra-carta'], type='http', auth="public", website=True)
    def selection_page(self, **post):
        ...
        default_category_names = [
            'Pasteles y Tartas', 'Bollería Creativa', 'Pastas de Té', 
            'Chocolates y Bombones', 'Postres de Temporada', 'Opciones Veganas/Sin Gluten'
        ]
        ...
        if post.get('exclude_allergens'):
            try:
                exclude_allergen_ids = [int(i) for i in post.get('exclude_allergens').split(',')]
            except ValueError:
                exclude_allergen_ids = []
        ...
        return request.render('cdv_website_card_products.nuestra_carta_page', values)
