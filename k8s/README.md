...
2. Se toma `common.yaml` como referencia estructural de imagen, base de datos y
   persistencia.
3. Se consideran varios elementos de `devel.yaml` como exclusivos de desarrollo
   y por tanto no se trasladan tal cual a Kubernetes de producción.
...
5. Los bind mounts de `./odoo/custom` y `./odoo/auto`, porque en Kubernetes el
   código debe ir dentro de la imagen construida por Doodba.
...
2. Rellenar `values-prod.example.yaml` con el dominio, namespace, imagen y storage class reales.
