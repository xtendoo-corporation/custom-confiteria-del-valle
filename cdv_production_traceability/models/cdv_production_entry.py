...
class CdvProductionEntry(models.Model):
    ...
    def _default_picking_type_id(self):
        ...
        picking_type_id = (
            ...
            .sudo()
            .get_param("cdv_production_traceability.finished_picking_type_id")
        )
        if picking_type_id:
        ...
        for record in self:
            if record.picking_type_id:
                ...
                record.location_dest_id = (
                    record.picking_type_id.default_location_dest_id
                )
            else:
                ...
                finished_location = (
                    self.env["ir.config_parameter"]
                    .sudo()
                    .get_param("cdv_production_traceability.finished_location_id")
                )
                record.location_dest_id = (
                    int(finished_location) if finished_location else False
                )
    ...
    def unlink(self):
        for record in self:
            if record.state == "done":
                raise UserError(
                    _("No puede eliminar un parte de producción confirmado.")
                )
        return super().unlink()
    ...
    def action_cancel(self):
        ...
        if self.picking_id and self.picking_id.state == "done":
            raise UserError(
                _("No puede cancelar un parte cuyo albarán ya está validado.")
            )
    ...
    def action_draft(self):
        ...
        if self.picking_id:
            raise UserError(
                _("No puede volver a borrador un parte que tiene albarán asociado.")
            )
    ...
    def _validate_bom_exists(self):
        ...
        if products_without_bom:
            raise ValidationError(
                _(
                    "Los siguientes productos no tienen lista de materiales definida:\n\n%s\n\n"
    ...
    def _create_incoming_picking(self):
        ...
        if not self.location_src_id or not self.location_dest_id:
            raise UserError(
                _(
                    "No se han configurado las ubicaciones de origen y destino.\n"
        ...
        auto_validate = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("cdv_production_traceability.auto_validate_picking", "False")
        )
