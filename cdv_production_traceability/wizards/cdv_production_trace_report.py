...
class CdvProductionTraceReport(models.TransientModel):
    ...
    def action_compute_trace(self):
        ...
        if not production_entries:
            raise UserError(_('No se encontraron partes de producción en el período seleccionado.'))
    ...
    def action_print_report(self):
        ...
        if self.state != 'computed':
            raise UserError(_('Debe calcular la trazabilidad antes de imprimir el informe.'))

        return self.env.ref('cdv_production_traceability.action_report_traceability').report_action(self)
    ...
    def action_export_excel(self):
        ...
        raise UserError(_('La exportación a Excel estará disponible en una versión futura.'))
